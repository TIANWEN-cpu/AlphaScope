...
- **许可证防火墙**：TradingAgents 项目 license 不明确 → 谨慎标 `COPILEFT_RISK` + `code_copy_allowed=False`（只通过 pip/外部进程用，不拷码入主仓，与规划 §12 License Firewall 一致）；`requires_evidence=True`。
...
- 许可证防火墙：项目 license 不明确 → 仅 pip/外部进程使用，不拷码入主仓。
