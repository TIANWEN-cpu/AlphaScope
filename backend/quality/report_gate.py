...
PLACEHOLDER_MARKERS = ["[TODO]", "TODO", "占位", "PLACEHOLDER", "[未实现]", "待补充", "xxx", "XXX"]
...
def check_fluff_phrases(ctx: dict) -> list[Issue]:
    ...
    for ph in FLUFF_CRITICAL:
        if ph in text:
            issues.append(Issue(
                severity="critical", category="fluff",
    ...
    for ph in FLUFF_WARNING:
        if ph in text:
            issues.append(Issue(
                severity="warning", category="fluff",
                issue=f"含模糊表述 {ph!r}",
                evidence=_snippet(text, ph),
...
def check_evidence_coverage(ctx: dict) -> list[Issue]:
    ...
    if cov < COVERAGE_CRIT:
        return [Issue(
            severity="critical", category="evidence",
    ...
    if cov < COVERAGE_WARN:
        return [Issue(
            severity="warning", category="evidence",
