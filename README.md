...
1. 打开 [GitHub Releases](https://github.com/TIANWEN-cpu/AlphaScope/releases)。
2. 下载 `AlphaScope-Setup-*.exe`。
3. 双击安装，安装完成后用桌面快捷方式启动。

安装包会自带运行时、后端服务和已构建的 Web 页面；用户不需要安装 Python、Node.js 或手动执行命令。首次启动会在安装目录创建 `.env`，需要使用 AI 分析时再填写模型 API Key。
...
没有安装 Inno Setup 时，`python build.py` 仍会生成 `dist/AlphaScope/AlphaScope.exe` 便携目录。
