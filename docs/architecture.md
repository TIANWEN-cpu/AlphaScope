...
- **因子注册中心 / 研究流水线** (`factor_registry.py`，v1.9.21) — 对标 Qlib factor/alpha 研究流程(保持确定性·失败安全·不触网基线):① **注册目录** `FactorDef` 统一登记 15+ 因子(10 个确定性技术因子 + 5 个已有软因子);② **确定性技术因子** 纯函数从 OHLCV 算 20/60 日动量、年化波动率、MA20/60 乖离、RSI、最大回撤、量比、距高点、区间位置(数据不足→None,失败安全、可单测);③ **缓存** 自包含懒建表 `factor_vectors`(仿 experiment_store);④ **研究流水线** `compute_for_symbol`(单标的向量)/ `compute_matrix`(跨标的因子矩阵,逐标的失败安全)。与既有 `factors/generator.py` 的软因子并存互补(后者登记入目录、数值仍由其计算)。`GET /api/factor-registry/{catalog,symbol/{id},cached/{id}}`、`POST .../matrix`。因子为历史量价度量,方向仅口径标注,不预测、不构成选股建议。
...
- **DuckDB / Parquet 数据湖** (`datalake.py`，v1.9.20) — 把零散的逐标的行情沉淀成**列式数据湖**(每标的一个 `data/datalake/prices/<symbol>.parquet`),用一条 SQL 跨上千标的批量扫描,作为批量选股 / 因子计算的底座。**`duckdb` 用 import-guard 包裹、零硬依赖**:没装照常运行(能力报告 `available=False`),装上即生效。纯函数 `normalize_bars` / `build_screen_sql`(字段·操作符白名单 + 占位符绑定**防 SQL 注入**)/ `is_select_only`(只读守卫)可单测;duckdb-gated 的 `ingest_prices`(写列存)、`ingest_from_provider`(复用价格面取数入湖)、`screen`(每标的最新一根 bar 批量筛选)、`query`(只读 SQL,表名 `prices`)、`latest_snapshot`/`stats` 全失败安全。写入仅在显式入湖时发生。`GET /api/datalake/{status,symbols,latest}`、`POST .../{ingest,screen,query}`、`DELETE .../{symbol/{id},all}`。批量筛选描述「过去满足条件的标的」,不预测、不构成选股建议。
...
- **`apps/web`** — Vite + React 19 + TypeScript 工作台，状态驱动 SPA（无 React Router），Sidebar 分两组（投研核心 / 量化研究引擎）约 20 个模块。SSE 流式对话、证据链反查、回测交易明细与免责声明、**低代码策略编辑器**（字段+操作符+阈值无代码组合信号）、**样本外走查 Tab**（IS/OOS 窗口稳健性体检，v1.9.5）、**筹码分布 / 策略榜 / 实验记录 Tab**（v1.9.6–1.9.8）、**策略进化 Tab**（遗传算法参数寻优 + 收敛曲线 + 一键转样本外走查，v1.9.11）、**系统监控中心**（v1.9.10）、**研究记忆**（v1.9.18，同一股票结论随时间变化轨迹）、**自定义数据表 TickFlow**（v1.9.19，HTTP/JSON 接口拉取）、**数据湖**（v1.9.20，列式批量选股/SQL）、**因子注册中心**（v1.9.21，因子目录/技术因子/因子矩阵）、研报页**多维图表分析**（v1.9.22，9 图:信号/置信度/多空/共识/阵营/形态/走势/量/因子)已接入。
