...
| 模块 | 内容 | 状态 |
|---|---|---|
| **M1** | A 股龙虎榜「机构 vs 游资」席位 + 杀猪盘信号(新 provider) | 进行中 |
| **M2** | DCF / Comps / LBO 机构估值建模(新 `backend/valuation/`) | 待开始 |
| **M3** | 研报质量机械门控(新 `backend/quality/report_gate.py`) | 待开始 |
| **M4** | 投资人 persona 扩充(`config/experts.yaml` + `prompts/experts/`) | 待开始 |
