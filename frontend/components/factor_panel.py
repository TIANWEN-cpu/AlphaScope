            ...
            with st.expander("📋 详细信号", expanded=False):
                for sig in report.signals:
                    ...
                    if sig_type == "news":
                        ...
                        st.write(f"📋 **[公告:{sig.get('category', '')}]** {sig.get('title', '')} — 得分: {sig.get('score', 0):+.2f}")
                    elif sig_type == "analyst":
                        st.write(f"📊 **[研报]** {sig.get('institution', '')}: {sig.get('title', '')} — 评级: {sig.get('rating_score', 0):+.2f}")
                    elif sig_type == "fund_flow":
                        st.write(f"💰 **[资金]** 主力净流入: {sig.get('last_main_yi', 0):.2f}亿, 流入{sig.get('inflow_days', 0)}天/流出{sig.get('outflow_days', 0)}天")
                    elif sig_type == "momentum":
                        st.write(f"📈 **[动量]** 近期涨幅: {sig.get('short_return_pct', 0):+.1f}%, 量能变化: {sig.get('volume_change_pct', 0):+.1f}%")

        # 样本统计
        st.caption(f"样本: 新闻 {report.news_count} 条 | 公告 {report.event_count} 条 | 研报 {report.report_count} 条 | 计算时间: {report.computed_at[:19]}")
