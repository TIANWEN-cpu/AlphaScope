...
from expert_panel import (  # noqa: E402
    run_roundtable, export_md, load_default_team, team_to_editable_dict,
    editable_dict_to_team, run_team_roundtable,
)
...
def _render_card(op, idx: int, symbol: str):
    ...
    if op.fallback_used and op.ok:
        fb_tag = f"<span style='background:#fef3c7; color:#b45309; font-size:0.66rem; padding:1px 6px; border-radius:4px; margin-left:4px;'>降级模型</span>"
    elif not op.ok:
        fb_tag = f"<span style='background:#fee2e2; color:#dc2626; font-size:0.66rem; padding:1px 6px; border-radius:4px; margin-left:4px;'>不可用</span>"
    ...
    with st.expander(f"查看 {op.expert_name} 完整观点与依据", expanded=False):
        ...
        if getattr(op, "risks", None):
            ...
            for r in op.risks:
                st.markdown(f"- {r}")
        st.markdown(f"**操作建议**: {op.action} | **建议仓位**: {op.position}% | "
...
def _render_team_editor(symbol: str) -> dict:
    ...
    with st.expander("专家团人设与成员管理", expanded=False):
        ...
        with t1:
            data["display_name"] = st.text_input("专家团名称", value=data.get("display_name", "自定义专家团"), key=f"team_name_{symbol}")
        with t2:
            data["avatar"] = st.text_input("团队头像", value=data.get("avatar", "🎓"), key=f"team_avatar_{symbol}")
        with t3:
            st.metric("启用专家数", sum(1 for m in data.get("members", []) if m.get("enabled", True)))
        ...
        for i, member in enumerate(list(data.get("members", []))):
            ...
            with st.expander(title, expanded=False):
                ...
                with c0:
                    member["enabled"] = st.checkbox("启用", value=member.get("enabled", True), key=f"expert_enabled_{symbol}_{i}")
                    member["avatar"] = st.text_input("头像", value=member.get("avatar", "🧠"), key=f"expert_avatar_{symbol}_{i}")
                with c1:
                    member["id"] = st.text_input("ID", value=member.get("id", f"expert_{i+1}"), key=f"expert_id_{symbol}_{i}")
                    member["name"] = st.text_input("名称", value=member.get("name", "自定义专家"), key=f"expert_name_{symbol}_{i}")
                with c2:
                    member["profession"] = st.text_input("职业/风格", value=member.get("profession", "投资专家"), key=f"expert_prof_{symbol}_{i}")
                    member["role"] = st.selectbox("角色", ["member", "lead"], index=1 if member.get("role") == "lead" else 0, key=f"expert_role_{symbol}_{i}")
                with c3:
                    member["card_style"] = st.selectbox("卡片样式", list(CARD_STYLES.keys()), index=list(CARD_STYLES.keys()).index(member.get("card_style", "default")) if member.get("card_style", "default") in CARD_STYLES else 0, key=f"expert_style_{symbol}_{i}")

                p1, p2 = st.columns([1, 1])
                with p1:
                    member["provider"] = st.text_input("Provider", value=member.get("provider", "deepseek"), key=f"expert_provider_{symbol}_{i}")
                with p2:
                    member["model"] = st.text_input("Model", value=member.get("model", "deepseek-chat"), key=f"expert_model_{symbol}_{i}")
                member["focus_dims"] = st.text_input("关注维度（逗号分隔）", value=member.get("focus_dims", ""), key=f"expert_focus_{symbol}_{i}")
                member["stop_loss_style"] = st.text_input("止损风格", value=member.get("stop_loss_style", "中等"), key=f"expert_stop_{symbol}_{i}")
                member["system_prompt"] = st.text_area("系统人设 Prompt", value=member.get("system_prompt", ""), height=120, key=f"expert_prompt_{symbol}_{i}")

                b1, b2 = st.columns([1, 1])
                with b1:
                    if st.button("复制该专家", use_container_width=True, key=f"expert_copy_{symbol}_{i}"):
                        clone = dict(member)
                        ...
                        st.rerun()
                with b2:
                    if st.button("删除该专家", use_container_width=True, key=f"expert_delete_{symbol}_{i}"):
                        data["members"].pop(i)
                        st.session_state[f"expert_team_config_{symbol}"] = data
                        st.rerun()
        st.session_state[f"expert_team_config_{symbol}"] = data
...
def render(symbol: str, stock_name: str, stock_brief: str):
    ...
    if run_btn:
        ...
        try:
            with st.spinner(f"{active_count} 位专家并行思考中..."):
                ...
                result = run_team_roundtable(team_cfg, stock_brief, stock_name, global_ai_settings=global_ai_settings)
    ...
    if not result:
        ...
        with st.expander("查看当前启用专家的人设介绍", expanded=False):
            for m in team_cfg.members:
                ...
                st.markdown(f"- **{m.avatar} {m.display_name}（{m.profession}）**: 关注 {dims}，止损风格：{m.stop_loss_style}")
        return
    ...
    order = result.get("member_order") or [m.id for m in team_cfg.members if getattr(m, "enabled", True)] or EXPERT_ORDER
    ...
    with ec1:
        if st.button("📥 导出纪要", use_container_width=True, key=f"rt_export_{symbol}"):
            try:
                ...
                arc = save_roundtable(stock_name, symbol, opinions, summary, md_text,
                                      dedupe_minutes=1)
