...
function IndividualFlow({ data }: { data: FundFlowData | null }) {
  if (!data || !data.summary) {
    return (
      <div className="flex flex-col items-center justify-center h-32 text-zinc-600">
        <AlertTriangle size={24} className="text-zinc-700 mb-2" />
        <p className="text-sm">暂无个股资金流向数据</p>
        <p className="text-[10px] text-zinc-700 mt-1">需akshare数据源可用</p>
      </div>
    );
  }
  ...
  const flowCards = [
    { label: "主力净流入(5日)", value: mainTotal, color: mainTotal >= 0 ? "text-red-400" : "text-emerald-400" },
    { label: "超大单", value: superTotal, color: superTotal >= 0 ? "text-red-400" : "text-emerald-400" },
    { label: "大单", value: largeTotal, color: largeTotal >= 0 ? "text-red-400" : "text-emerald-400" },
    { label: "中单", value: mediumTotal, color: mediumTotal >= 0 ? "text-red-400" : "text-emerald-400" },
    { label: "小单", value: smallTotal, color: smallTotal >= 0 ? "text-red-400" : "text-emerald-400" },
  ];
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-[#18181b] p-3 rounded-lg border border-zinc-800/50">
        <div className="flex items-center justify-between">
          ...
          <span className={`text-sm font-medium flex items-center gap-1 ${
            mainTotal > 0.5 ? "text-red-400" : mainTotal < -0.5 ? "text-emerald-400" : "text-zinc-400"
          }`}>
      ...
      {data.records && data.records.length > 0 && (
        <div className="bg-[#18181b] p-3 rounded-lg border border-zinc-800/50">
          <div className="text-xs text-zinc-500 mb-2">近期主力净流入趋势</div>
            ...
            {data.records.slice(-20).map((r, i) => {
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center gap-0.5">
                  <div
                    className={`w-full rounded-t ${r.main_net_yi >= 0 ? "bg-red-500/60" : "bg-emerald-500/60"}`}
      ...
      {data.records && data.records.length > 0 && (
        <div className="bg-[#18181b] rounded-lg border border-zinc-800/50 overflow-hidden">
          <div className="text-xs text-zinc-500 p-3 pb-0">每日明细</div>
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-zinc-800/50">
                  <th className="px-3 py-2 text-left text-zinc-500 font-normal">日期</th>
                  <th className="px-3 py-2 text-right text-zinc-500 font-normal">收盘</th>
                  <th className="px-3 py-2 text-right text-zinc-500 font-normal">涨跌%</th>
                  <th className="px-3 py-2 text-right text-zinc-500 font-normal">主力净额</th>
                  <th className="px-3 py-2 text-right text-zinc-500 font-normal">超大单</th>
                  <th className="px-3 py-2 text-right text-zinc-500 font-normal">大单</th>
                  <th className="px-3 py-2 text-right text-zinc-500 font-normal">中单</th>
                  <th className="px-3 py-2 text-right text-zinc-500 font-normal">小单</th>
                </tr>
              </thead>
              <tbody>
                {data.records.slice(-15).reverse().map((r, i) => (
                  <tr key={i} className="border-b border-zinc-800/30 hover:bg-zinc-800/20">
                    <td className="px-3 py-1.5 text-zinc-400 font-mono">{r.date}</td>
                    <td className="px-3 py-1.5 text-right text-zinc-300 font-mono">{r.close?.toFixed(2)}</td>
                    <td className={`px-3 py-1.5 text-right font-mono ${r.change_pct >= 0 ? "text-red-400" : "text-emerald-400"}`}>
                      {r.change_pct >= 0 ? "+" : ""}{r.change_pct?.toFixed(2)}%
                    </td>
                    <td className={`px-3 py-1.5 text-right font-mono ${r.main_net_yi >= 0 ? "text-red-400" : "text-emerald-400"}`}>
                      {r.main_net_yi >= 0 ? "+" : ""}{r.main_net_yi?.toFixed(2)}亿
                    </td>
                    <td className={`px-3 py-1.5 text-right font-mono ${r.super_net_yi >= 0 ? "text-red-400" : "text-emerald-400"}`}>
                      {r.super_net_yi >= 0 ? "+" : ""}{r.super_net_yi?.toFixed(2)}
                    </td>
                    <td className={`px-3 py-1.5 text-right font-mono ${r.large_net_yi >= 0 ? "text-red-400" : "text-emerald-400"}`}>
                      {r.large_net_yi >= 0 ? "+" : ""}{r.large_net_yi?.toFixed(2)}
                    </td>
                    <td className={`px-3 py-1.5 text-right font-mono ${r.medium_net_yi >= 0 ? "text-red-400" : "text-emerald-400"}`}>
                      {r.medium_net_yi >= 0 ? "+" : ""}{r.medium_net_yi?.toFixed(2)}
                    </td>
                    <td className={`px-3 py-1.5 text-right font-mono ${r.small_net_yi >= 0 ? "text-red-400" : "text-emerald-400"}`}>
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-4 gap-3">
        {cards.map((card, i) => (
          <div key={i} className="bg-[#18181b] p-3 rounded-lg border border-zinc-800/50">
            <div className="text-[10px] text-zinc-500 mb-1">{card.label}</div>
            <div className={`text-sm font-mono font-medium ${card.value >= 0 ? "text-red-400" : "text-emerald-400"}`}>
      ...
      {data.records && data.records.length > 0 && (
        <div className="bg-[#18181b] p-3 rounded-lg border border-zinc-800/50">
          <div className="text-xs text-zinc-500 mb-2">大盘主力资金趋势</div>
          <div className="flex items-end gap-1 h-24">
            {data.records.slice(-20).map((r, i) => {
              ...
              return (
                <div key={i} className="flex-1 flex flex-col items-center gap-0.5">
                  <div
                    className={`w-full rounded-t ${val >= 0 ? "bg-red-500/60" : "bg-emerald-500/60"}`}
                    style={{ height: `${Math.min(h, 80)}px` }}
                  />
                  <div className="text-[7px] text-zinc-600 font-mono">
