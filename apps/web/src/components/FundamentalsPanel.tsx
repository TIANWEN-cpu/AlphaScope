...
function OverviewTab({ data }: { data: any }) {
  if (!data) {
    return <div className="text-zinc-600 text-sm text-center py-8">暂无财务数据</div>;
  }
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-4 gap-3">
        {metricCards.map((card, i) => (
          <div key={i} className="bg-[#18181b] p-3 rounded-lg border border-zinc-800/50">
            <div className="text-[10px] text-zinc-500 mb-1">{card.label}</div>
            <div className="text-sm font-mono text-zinc-200">{card.value}</div>
          </div>
        ))}
      </div>
      ...
      {score.dimension_scores && (
        <div className="bg-[#18181b] p-3 rounded-lg border border-zinc-800/50">
          <div className="text-xs text-zinc-500 mb-2">评分维度</div>
          <div className="grid grid-cols-4 gap-2">
            {Object.entries(score.dimension_scores as Record<string, number>).map(([key, val]) => (
              <div key={key} className="text-center">
                <div className="text-[10px] text-zinc-600 mb-1">{key}</div>
                <div className="text-sm font-mono text-zinc-300">{String(val)}</div>
      ...
      {periods.length > 0 && (
        <div className="bg-[#18181b] p-3 rounded-lg border border-zinc-800/50">
          <div className="text-xs text-zinc-500 mb-2">近四期财务趋势</div>
          <div className="grid grid-cols-4 gap-2">
            {periods.slice(0, 4).map((p: any, i: number) => (
              <div key={i} className="bg-[#09090b] p-2 rounded border border-zinc-800/50 text-center">
                <div className="text-[10px] text-zinc-600">{String(p.period || "")}</div>
                ...
                <div className={`text-[10px] font-mono ${
                  Number(p.yoy_revenue_pct || 0) >= 0 ? "text-red-400" : "text-emerald-400"
                }`}>
...
function ShareholdersTab({ data }: { data: any }) {
  if (!data) {
    return <div className="text-zinc-600 text-sm text-center py-8">暂无股东数据</div>;
  }
  ...
  const sections = [
    { title: "十大股东", data: topHolders, icon: <Users size={14} className="text-blue-400" /> },
    { title: "十大流通股东", data: circHolders, icon: <Users size={14} className="text-purple-400" /> },
    { title: "机构变动", data: instChanges, icon: <Building2 size={14} className="text-orange-400" /> },
  ];

  return (
    <div className="space-y-4">
      {sections.map((section, si) => (
        <div key={si} className="bg-[#18181b] rounded-lg border border-zinc-800/50 overflow-hidden">
          <div className="flex items-center gap-2 p-3 border-b border-zinc-800/50">
            {section.icon}
            <span className="text-xs text-zinc-300 font-medium">{section.title}</span>
            <span className="text-[10px] text-zinc-600">({section.data.length}人)</span>
          </div>
          {section.data.length === 0 ? (
            <div className="p-4 text-xs text-zinc-600 text-center">暂无数据</div>
          ) : (
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-zinc-800/30">
                    ...
                    <th className="px-3 py-2 text-left text-zinc-500 font-normal">股东名称</th>
                    <th className="px-3 py-2 text-right text-zinc-500 font-normal">持股(亿股)</th>
                    <th className="px-3 py-2 text-right text-zinc-500 font-normal">持股比例</th>
                    <th className="px-3 py-2 text-center text-zinc-500 font-normal">变动</th>
                  </tr>
                </thead>
                <tbody>
                  {section.data.map((h: any, i: number) => (
                    <tr key={i} className="border-b border-zinc-800/30 hover:bg-zinc-800/20">
                      <td className="px-3 py-1.5 text-zinc-500">{Number(h.rank || i + 1)}</td>
                      <td className="px-3 py-1.5 text-zinc-300">{String(h.name || "")}</td>
...
function PeersTab({ data }: { data: any }) {
  if (!data) {
    return <div className="text-zinc-600 text-sm text-center py-8">暂无行业对比数据</div>;
  }
  ...
  return (
    <div className="space-y-4">
      <div className="bg-[#18181b] p-3 rounded-lg border border-zinc-800/50">
        <div className="text-xs text-zinc-500 mb-1">所属行业</div>
      ...
      {peerList.length > 0 && (
        <div className="bg-[#18181b] rounded-lg border border-zinc-800/50 overflow-hidden">
          <div className="p-3 border-b border-zinc-800/50 text-xs text-zinc-500">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-zinc-800/30">
                  <th className="px-3 py-2 text-left text-zinc-500 font-normal">代码</th>
                  <th className="px-3 py-2 text-left text-zinc-500 font-normal">名称</th>
                  <th className="px-3 py-2 text-right text-zinc-500 font-normal">总市值(亿)</th>
                  <th className="px-3 py-2 text-right text-zinc-500 font-normal">PE</th>
                  <th className="px-3 py-2 text-right text-zinc-500 font-normal">PB</th>
                  <th className="px-3 py-2 text-center text-zinc-500 font-normal">标记</th>
                </tr>
              </thead>
              <tbody>
                {peerList.map((p: any, i: number) => (
                  <tr key={i} className={`border-b border-zinc-800/30 hover:bg-zinc-800/20 ${p.is_self ? "bg-blue-500/5" : ""}`}>
                    <td className="px-3 py-1.5 text-zinc-400 font-mono">{String(p.symbol || "")}</td>
                    <td className="px-3 py-1.5 text-zinc-300">{String(p.name || "")}</td>
                    <td className="px-3 py-1.5 text-right text-zinc-300 font-mono">
