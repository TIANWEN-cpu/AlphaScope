  ...
  if (portfolios.length === 0) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center gap-3 text-zinc-500">
        ...
        <p className="text-xs text-zinc-600">点击「新建」创建第一个组合</p>
      </div>
    );
  }
...
function PortfolioCard({
  ...
  return (
    <div className="bg-zinc-800/40 rounded border border-zinc-800/60 p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex gap-1">
          <button
            ...
            title="再平衡"
            className="p-1.5 rounded hover:bg-zinc-700 text-zinc-400 hover:text-blue-400 disabled:opacity-50"
          >
            {rebalancing ? (
          ...
          <button
            ...
            disabled={deleting}
            title="删除"
            className="p-1.5 rounded hover:bg-zinc-700 text-zinc-400 hover:text-red-400 disabled:opacity-50"
          >
            {deleting ? (
              <Loader2 size={14} className="animate-spin" />
            ) : (
      ...
      {portfolio.holdings.length === 0 ? (
        <div className="text-xs text-zinc-600 py-2">无持仓</div>
      ) : (
          ...
          <div className="grid gap-1">
            {trades.map((t, i) => (
              <div
                ...
                className="flex items-center justify-between bg-zinc-900/50 rounded px-2 py-1.5"
              >
                <span className="text-xs text-zinc-300">{t.fund_code}</span>
                <span
                  className={`text-xs ${
                    t.action === "buy"
                      ? "text-emerald-400"
  ...
  return (
    <div className="bg-zinc-800/30 rounded-lg border border-zinc-700 p-4">
      ...
      <div className="grid gap-2 mb-3">
        <div>
          <label className="text-xs text-zinc-500 mb-1 block">组合名称</label>
          <input
            value={name}
      ...
      <div className="mb-3">
        <label className="text-xs text-zinc-500 mb-1 block">添加持仓</label>
        <div className="flex gap-2">
          <input
            value={fundCode}
            onChange={(e) => setFundCode(e.target.value)}
            placeholder="基金代码"
            className="flex-1 bg-zinc-800 border border-zinc-700 rounded px-2 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-blue-500"
          />
          <div className="flex items-center gap-1">
            <input
              ...
              placeholder="权重"
              className="w-16 bg-zinc-800 border border-zinc-700 rounded px-2 py-1.5 text-sm text-zinc-200 focus:outline-none focus:border-blue-500"
            />
        ...
        <div className="mb-3 grid gap-1">
          {holdings.map((h, i) => (
            <div
              ...
              className="flex items-center justify-between bg-zinc-900/50 rounded px-2 py-1.5"
            >
              <span className="text-xs text-zinc-300">{h.fund_code}</span>
