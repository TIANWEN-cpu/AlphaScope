...
class TestValidateAgentOutput:
    def test_full_valid_payload_passes_through(self):
        payload = {
            ...
            "confidence": 75,
            "reason": "基本面+主力资金双重利好",
            "evidence": [{"type": "fund_flow", "claim": "近5日主力净流入2.3亿", "data_date": "2026-05-16"}],
    ...
    def test_evidence_empty_claim_dropped(self):
        out = validate_agent_output({
            "evidence": [{"type": "technical", "claim": "   "}, {"type": "fund_flow", "claim": "净流入"}],
        })
...
class TestValidateExpertOutput:
    ...
    @pytest.mark.parametrize("raw,expected", [
        ...
        ("建议卖出", "卖出"),
        ("观望", "观望"),
        ("减仓", "观望"),  # not on whitelist, not a substring of any action -> default
        ("减持", "减持"),
        ("",     "观望"),
        ("foo",  "观望"),
    ])
