...
def test_serialize_evidence_dict_with_type_and_date():
    out = _serialize_evidence([
        {"type": "fund_flow", "claim": "近5日主力净流入2.3亿", "data_date": "2026-05-16"},
...
def test_prompt_includes_keys_signals_and_evidence():
    agents = {
        "fundamental": {
            "name": "🏛️ 基本面分析师",
            "signal": "买入", "confidence": 78, "reason": "高 ROE",
        ...
        "technical": {
            "name": "📐 技术分析师",
            "signal": "卖出", "confidence": 60, "reason": "短期超买",
        },
    }
...
def test_parse_filters_unknown_keys_and_clamps_score():
    raw = json.dumps({
        "agents": [
            {"key": "fundamental", "quality_score": 999, "supported": ["a"], "contradictions": [], "missing_evidence": [], "overconfident": False, "comment": "ok"},
            {"key": "ghost", "quality_score": 50, "comment": "应被过滤"},
            {"key": "technical", "quality_score": -10, "comment": "下界裁剪"},
        ],
        "divergence": {"level": "中", "main_axis": "估值 vs 技术", "summary": "短期与长期分歧"},
    }, ensure_ascii=False)
